...
* check current repo status with:
```
git status
```
* start to track all files with:
```
git add -A
```
* check current repo status and confirm all files are tracked:
```
git status
```
* commit with some comment:
```
git commit -m "some comment"
```
* check repo status to confirm the commit is done:
```
git status
```
* demo an accident of deletion:
```
rm -rf config
```
* check current repo status after deletion:
```
git status
```
...
git checkout -f
```
* check repo status after recovery with:
```
git status
```
...
1. as Heroku does not support sqlite3, check its gem is in dev group in Gemfile
2. add a production group in Gemfile, and add gem 'pg' within this group
3. update Gemfile.lock with:
```
...
10. optionally, app name could be renamed:
```
...
```
2. go to db/migrate, update the change method in newly created migration file:
```
def change
...
t.string :email
t.timestamps
3) deploy migration again: rails db:migrate
